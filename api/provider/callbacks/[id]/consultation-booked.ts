...
import { providerConvertConsultation } from '../../../../api-lib/services/callbackService.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (method === 'PATCH') {
    try {
      const updated = await providerConvertConsultation(paramId, user.id);
      return res.status(200).json({
        success: true,
        message: 'Lead status updated to CONSULTATION_BOOKED.',
        data: updated,
      });
    } catch (err: any) {
