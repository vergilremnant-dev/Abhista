...
import { createRequirement } from '../../services/requirementService.js';
...
export default async function handler(req, res) {
    ...
    if (method === 'POST') {
        ...
        try {
            ...
            const { title, description, serviceCategory, serviceCategoryId, location, budgetMin, budgetMax, preferredStartDate, status, attachments, } = req.body;
    ...
    if (method === 'GET') {
        ...
        const { search, categoryId, location, status, minBudget, maxBudget, limit = '10', cursor, } = req.query;
        ...
        try {
            ...
            if (userProfile?.role === 'PROVIDER') {
                whereClause.status = status ? status : RequirementStatus.PUBLISHED;
            }
